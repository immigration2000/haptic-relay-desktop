...
첫 번째 앱에서 `스트리머`를 선택하고 자유입장 방을 만든 뒤, 두 번째 앱에서 `시청자`를 선택해 같은 방에 입장합니다. 스트리머 앱의 `모션 테스트`에서 위치와 강도를 조절해 전송하면 시청자 앱의 `관리자 수신 모니터`에 같은 값이 표시됩니다. 하드웨어가 없는 경우 `가상 수신 정상 / 하드웨어 미연결`이 표시되면 실제 릴레이 수신 경로가 정상입니다.
...
2. 노트북 앱의 `방 찾기`에 실제 방이 표시되는지 확인합니다. 목록은 화면 진입과 서버 선택 직후, 이후 3초마다 갱신됩니다.
3. 표시된 방 카드를 눌러 시청자 역할로 입장합니다. 목록 조회가 실패하면 새로고침 아이콘을 누르거나 초대 코드로 직접 입장할 수 있습니다.
4. 먼저 장비 없이 모션 전송, 긴급 정지, 승인/강퇴 흐름을 확인합니다.
5. 장비를 연결한 뒤 낮은 강도와 제한된 위치 범위로 하드웨어 테스트를 실행합니다.
...
- `sequence`: 실제 네트워크 송신 프레임 기준으로 증가하는 uint32 순번
...
연결 중에는 main process에 적용된 프로필과 화면 값이 달라지지 않도록 프로필 변경과 설정 불러오기를 잠급니다. 프로필을 바꾸려면 먼저 직렬 포트를 닫습니다. UI에서는 이 동작을 `연결 해제`로 표시합니다. 이 버튼은 `DSTOP`이나 위치 명령을 보내지 않고 포트만 닫으며, 기존 emergency latch 상태를 바꾸지 않습니다.
...
같은 스트리머 값이 유지되거나 새 패킷이 잠시 없으면 장비는 마지막 명령 위치를 유지합니다. 자동 inactivity 정지 위치 이동은 없습니다. 절대 정지 위치는 방 나가기 또는 긴급정지에서만 사용합니다. 긴급정지는 로컬 잠금이며 사용자가 **긴급정지 해제**를 눌러야 다시 움직입니다. 해제는 해당 앱에만 적용되고 해제 순간에는 모션 명령을 보내지 않으므로, 이후 새로 수신한 유효 프레임부터 출력할 수 있습니다.
