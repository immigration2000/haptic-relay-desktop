...
2. `로그인`을 누릅니다.
3. `방 찾기` 로비와 샘플 방 3개가 표시되는지 확인합니다.

현재 로그인은 화면 시연용 로컬 로그인입니다. 비밀번호는 저장하지 않으며 실제 계정 서버를 호출하지 않습니다. 로비의 `데모 데이터` 방도 UI 확인용이며 실제 릴레이, 모션 전송, 하드웨어를 호출하지 않습니다.
...
1. 로비 오른쪽 위의 `초대 코드`를 누릅니다.
2. 스트리머가 복사한 `HRS1...` 코드를 붙여 넣고 `초대 코드 적용`을 누릅니다. 코드를 사용하지 않을 때는 서버 URL, 표시 이름, 방 이름, 비밀번호를 직접 입력합니다.
3. 표시 이름을 입력합니다. 예: `viewer-01`
4. `입장 요청`을 누릅니다.
5. `PARTICIPANT SESSION`과 `수신 모니터`가 표시되는지 확인합니다.
6. 스트리머의 `방 관리`에 접속자 1명이 표시되는지 확인합니다.

## 10. 모션 전송 확인

1. 스트리머 세션에서 `실시간 시연` 탭을 엽니다.
2. `시연 시작`을 누릅니다.
3. 위치와 강도 슬라이더를 마우스로 계속 조절합니다.
4. 시연 중 최신 위치와 강도가 30Hz로 계속 전송되는지 확인합니다.
5. 시청자 창의 `관리자 수신 모니터`를 확인합니다.
6. 확인이 끝나면 `시연 중지`를 누릅니다.
