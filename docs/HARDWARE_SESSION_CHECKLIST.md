...
5. 하드웨어 연결 해제 시 절대 정지 위치로 이동하지 않고 포트만 닫히는지 확인합니다.
   ...
   - counter delta가 `0`이고 새 `kind=stop` event나 `DSTOP`이 없으며, 장비가 절대 정지 위치로 이동하지 않은 채 포트만 닫히는지 확인합니다.
   - 다음 항목 전에 COM3를 명시적으로 다시 연결합니다. 새 motion은 보내지 않고 non-stop 위치가 유지되는지 확인합니다.
