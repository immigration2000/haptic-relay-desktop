...
electron/main.ts
  Electron main process. Renderer? native ?? ?? IPC ??, CSP, navigation/window-open ??, IPC ??? ??, settings.json ??/????, ?? ??? ?? ??.
...
renderer? `contextIsolation: true`, `nodeIntegration: false`, `sandbox: true`? ?????. preload? ??? ??? `window.hapticRelay`? ????, main process? ?? IPC ???? sender? ?? main window?? ?????.
...
`HAPTIC_HARDWARE_SAFETY_TIMEOUT_MS`? ? motion frame? ???? ?? ? ?? ??? ??????? ?????. `0` ??? ???? safety timeout? ???????.
...
?? motion frame? ???? ??????? ?? `0.0-1.0` ??? ?? ?????. ???? ???? SerialPort ?? ???? ?????.
...
`receive pause`? ??? ?? ?? ?? `DSTOP`? ????, ?? ?? motion frame? ???? queue? ?? ????. relay room ?? ??? ????? ???? ????? ??? ? ?? ??? ? ????.
...
renderer? ?? ? `app:get-settings` IPC? ??? ??, `app:save-settings` IPC? ?? ?? ?????. main process? ?? ? `HardwareProfile`, `HardwareProtection`? ?? ?????. ?? ??? ??? JSON? ?? ?? ????? ?????.

## 11.2 ??? ??

main process? ?? 300? ???? ??? ??? ?????. renderer? ?? ? `app:logs` IPC? ?? ??? ??, ?? `app:log` push event? ?? UI? ?? 80?? ?????.
